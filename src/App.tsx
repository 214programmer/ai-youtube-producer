...
import { Loader2, Search, Youtube, Zap, AlertTriangle, Lightbulb, ExternalLink, HelpCircle, FileText, DollarSign, TrendingUp, BarChart3, Users, ChevronRight, LayoutGrid, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#020203] text-zinc-100 p-4 md:p-12 font-sans selection:bg-emerald-500/20">
      <div className="max-w-7xl mx-auto space-y-12">
        <header className="flex items-center space-x-6 border-b border-zinc-800/40 pb-12">
          <div className="bg-emerald-500 p-4 rounded-3xl text-black shadow-lg shadow-emerald-500/30"><Youtube size={40} /></div>
          <div>
            <h1 className="text-4xl font-black italic tracking-tighter uppercase leading-none">AI Producer <span className="text-emerald-500">Ultra</span></h1>
            <p className="text-zinc-500 font-bold text-[10px] tracking-[0.4em] mt-2 uppercase">Precision Channel Intelligence</p>
          </div>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[50px] space-y-6 shadow-2xl sticky top-12">
              <input placeholder="Ссылка или @ник" value={channelUrl} onChange={(e)=>setChannelUrl(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-3xl p-5 text-sm outline-none focus:border-emerald-500 transition-all" />
              <input placeholder="Ниша" value={niche} onChange={(e)=>setNiche(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-3xl p-5 text-sm outline-none focus:border-emerald-500 transition-all" />
              <input type="password" placeholder="Ключ Groq" value={customGeminiKey} onChange={(e)=>setCustomGeminiKey(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-3xl p-5 text-sm outline-none focus:border-emerald-500 transition-all" />
              <button onClick={handleAnalyze} disabled={isLoading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-black font-black py-5 rounded-2xl flex items-center justify-center transition-all shadow-xl uppercase tracking-widest text-[10px]">
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-zinc-900/40 border border-zinc-800 p-8 rounded-[45px] text-center backdrop-blur-md group hover:border-emerald-500/50 transition-all duration-500">
                ...
                <div className="relative p-12 bg-gradient-to-br from-emerald-500/10 via-black to-zinc-900 border border-emerald-500/20 rounded-[60px] shadow-2xl overflow-hidden group">
                    <Zap className="absolute -top-10 -right-10 w-48 h-48 text-emerald-500/5 rotate-12" />
                    ...
                    <p className="text-zinc-300 text-xl leading-relaxed italic border-l-4 border-emerald-500/30 pl-10">{data.aiAnalysis?.bestVideoAnalysis}</p>
                </div>
                ...
                {!deepData && (
                    <button onClick={handleDeepReport} disabled={isDeepLoading} className="w-full bg-violet-600 hover:bg-violet-500 h-24 rounded-[50px] flex items-center justify-center font-black italic uppercase text-2xl shadow-xl shadow-violet-900/30 transition-all animate-pulse">
                        {isDeepLoading ? <Loader2 className="animate-spin mr-5 w-10 h-10"/> : <LayoutGrid className="mr-5 w-10 h-10"/>} ГЛУБОКАЯ СТРАТЕГИЯ
                    </button>
                )}
                    ...
                    <div className="space-y-6">
                        ...
                        {data.aiAnalysis.mistakes.map((m:string, i:number) => (
                            <div key={i} className="p-8 bg-zinc-900/30 border border-zinc-800 rounded-[50px] space-y-4 shadow-xl hover:border-red-500/30 transition-all">
                                <p className="text-sm text-zinc-300 font-bold italic leading-relaxed">“ {m} ”</p>
                                <button onClick={()=>handleExplain(m)} className="flex items-center text-[9px] font-black text-red-500 uppercase hover:text-red-400 tracking-widest pl-1 transition-colors">
                                    <ChevronRight size={14} className="mr-1"/> {explanations[m]?.loading ? "Пишу..." : "Анализ проблемы"}
                                </button>
                                {explanations[m]?.text && (
                                    <div className="mt-4 p-6 bg-black/60 rounded-[30px] border border-red-500/10 text-[13px] text-zinc-400 leading-relaxed animate-in zoom-in-95">{explanations[m].text}</div>
                                )}
                            </div>
                        ))}
                    </div>
                    <div className="space-y-6">
                        <h4 className="text-emerald-500 font-black uppercase text-xs tracking-[0.5em] ml-8 mb-4">Стратегия Роста</h4>
                        {data.aiAnalysis.tips.map((t:string, i:number) => (
                            <div key={i} className="p-8 bg-zinc-900/30 border border-zinc-800 rounded-[50px] space-y-4 shadow-xl hover:border-emerald-500/30 transition-all">
                                <p className="text-sm text-zinc-300 font-bold leading-relaxed">{t}</p>
                                <button onClick={()=>handleExplain(t)} className="flex items-center text-[9px] font-black text-emerald-500 uppercase hover:text-emerald-400 tracking-widest pl-1 transition-colors">
                                    <CheckCircle2 size={14} className="mr-1"/> {explanations[t]?.loading ? "Готовлю..." : "Как внедрить?"}
                                </button>
                                {explanations[t]?.text && (
                                    <div className="mt-4 p-6 bg-black/60 rounded-[30px] border border-emerald-500/10 text-[13px] text-zinc-400 leading-relaxed animate-in zoom-in-95">{explanations[t].text}</div>
                                )}
                            </div>
                        ))}
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {deepData.contentPlan.map((p:any, i:number) => (
                                <div key={i} className="p-10 bg-zinc-900 border border-zinc-800 rounded-[60px] space-y-5 shadow-2xl hover:border-violet-500/30 transition-all">
                                    <span className="bg-violet-600 text-white px-5 py-2 rounded-full font-black text-[9px] uppercase tracking-widest shadow-lg shadow-violet-900/50">ДЕНЬ {p.day}</span>
                                    <p className="text-xl text-zinc-100 font-black leading-tight italic">{p.topic}</p>
                                    <button onClick={()=>handleExplain(p.topic)} className="text-[10px] font-black text-violet-400 uppercase flex items-center hover:text-white transition-colors"><HelpCircle size={14} className="mr-2"/> Разбор идеи</button>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {data.outlierVideos.map((v:any, i:number) => (
                            <a key={i} href={v.url} target="_blank" rel="noreferrer" className="group block bg-[#0f0f11] border border-zinc-800 rounded-[45px] overflow-hidden hover:border-emerald-500 transition-all duration-500 shadow-xl">
                                ...
                                <div className="p-6"><p className="text-[11px] font-black line-clamp-2 text-zinc-200 leading-snug">{v.title}</p></div>
                            </a>
                        ))}
                    </div>
                </div>
