    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 p-4 md:p-10 font-sans leading-relaxed">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <header className="flex items-center space-x-4 border-b border-zinc-800 pb-8">
          <div className="bg-emerald-500 p-2 rounded-xl text-black"><Youtube size={32} /></div>
          <h1 className="text-3xl font-black italic tracking-tighter uppercase">AI PRODUCER <span className="text-emerald-500">MAX V3</span></h1>
        </header>
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    {l: "Видео", v: data.channelData.videoCount, i: <Youtube size={16}/>},
                ...
                <div className="p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-[40px] shadow-2xl relative">
                    ...
                    <p className="text-zinc-300 italic whitespace-pre-line">{data.aiAnalysis.bestVideoAnalysis}</p>
                </div>
                ...
                <div className="space-y-4">
                    <h3 className="text-xl font-black italic text-emerald-500 uppercase tracking-tighter flex items-center"><TrendingUp className="mr-2 w-5 h-5"/> Видео конкурентов (Клик)</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {data.outlierVideos.map((v:any, i:number) => (
                            <a key={i} href={v.url} target="_blank" rel="noreferrer" className="group block bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden hover:border-emerald-500 transition-all shadow-xl">
                                ...
                                <div className="p-4"><p className="text-[10px] font-bold line-clamp-2 text-zinc-200">{v.title}</p></div>
                            </a>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {data.aiAnalysis.mistakes.map((m:string, i:number) => (
                            <div key={i} className="p-5 bg-[#18181b] border border-zinc-800 rounded-3xl space-y-3">
                                ...
                                <button onClick={()=>handleExplain(m)} className="text-[9px] font-black text-red-500 uppercase hover:text-red-400">
                    ...
                    <div className="space-y-4">
                        ...
                        {data.aiAnalysis.tips.map((t:string, i:number) => (
                            <div key={i} className="p-5 bg-[#18181b] border border-zinc-800 rounded-3xl space-y-3">
                                ...
                                <button onClick={()=>handleExplain(t)} className="text-[9px] font-black text-emerald-400 uppercase hover:text-emerald-300">
                    ...
                    <div className="space-y-10 pt-10 border-t border-zinc-800 animate-in slide-in-from-bottom-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] space-y-4 shadow-2xl">
                                <h4 className="text-emerald-500 font-black uppercase text-[10px] tracking-widest flex items-center"><DollarSign className="mr-2 w-4 h-4"/> Монетизация</h4>
                                {deepData.monetization.map((m:string, idx:number) => <p key={idx} className="text-sm text-zinc-400 border-b border-zinc-800/50 pb-3">• {m}</p>)}
                            </div>
                            <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] space-y-4 shadow-2xl">
                                <h4 className="text-blue-400 font-black uppercase text-[10px] tracking-widest flex items-center"><Zap className="mr-2 w-4 h-4"/> SEO Теги</h4>
