          ...
          <div className="lg:col-span-3 space-y-12">
            {data && (
              <>
                ...
                {!deepData && (
                    <button onClick={handleDeepReport} disabled={isDeepLoading} className="w-full bg-violet-600 hover:bg-violet-500 h-24 rounded-[40px] flex items-center justify-center font-black italic uppercase text-2xl shadow-xl shadow-violet-900/20 animate-pulse transition-all">
                    ...
                    <div className="space-y-6">
                        ...
                        {(data.aiAnalysis?.mistakes || []).map((m:string, i:number) => (
                            <div key={i} className="p-8 bg-[#0f0f11] border border-zinc-800 rounded-[40px] space-y-5 shadow-xl transition-all">
                                ...
                                {explanations[m]?.text && (
                                    <div className="mt-4 p-7 bg-black/60 rounded-[30px] border border-red-500/10 text-[13px] text-zinc-400 leading-relaxed whitespace-pre-line animate-in zoom-in-95">{explanations[m].text}</div>
                                )}
                            </div>
                        ))}
                    </div>
                    <div className="space-y-6">
                        ...
                        {(data.aiAnalysis?.tips || []).map((t:string, i:number) => (
                            <div key={i} className="p-8 bg-[#0f0f11] border border-zinc-800 rounded-[40px] space-y-5 shadow-xl transition-all">
                                ...
                                {explanations[t]?.text && (
                                    <div className="mt-4 p-7 bg-black/60 rounded-[30px] border border-emerald-500/10 text-[13px] text-zinc-400 leading-relaxed whitespace-pre-line animate-in zoom-in-95">{explanations[t].text}</div>
                                )}
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {(data.outlierVideos || []).map((v:any, i:number) => (
                            <a key={i} href={v.url} target="_blank" rel="noreferrer" className="group block bg-[#0f0f11] border border-zinc-800 rounded-[35px] overflow-hidden hover:border-emerald-500 transition-all duration-500 shadow-xl">
                                ...
                                <div className="p-5 flex justify-between items-center"><p className="text-[11px] font-black line-clamp-2 text-zinc-200 leading-snug">{v.title}</p><ExternalLink size={12}/></div>
                            </a>
                        ))}
                    </div>
                </div>
